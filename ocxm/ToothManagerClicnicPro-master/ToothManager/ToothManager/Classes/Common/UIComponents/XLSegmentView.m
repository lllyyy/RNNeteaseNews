...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titleArray clickBlick:(BtnClickBlock)block{
    if (self = [super initWithFrame:frame]) {
        ...
        _bgSelectColor = [UIColor whiteColor];
        //初始化scrollView
        ...
        for (int i=0; i<titleArray.count; i++) {
            ...
            [btn setBackgroundColor:_bgNormalColor];
...
- (void)setUpSubViewFrame{
    ...
    _btn_w = 0.0;
    if (self.titleArray.count < MAX_TitleNumInWindow + 1) {
        _btn_w = kWindowWidth / self.titleArray.count;
    }else{
        _btn_w = kWindowWidth / MAX_TitleNumInWindow;
    }
    //初始化scrollView
    ...
    _bgScrollView.backgroundColor = [UIColor whiteColor];
    _bgScrollView.showsHorizontalScrollIndicator = NO;
    _bgScrollView.contentSize = CGSizeMake(_btn_w * self.titleArray.count, self.frame.size.height);
    [self addSubview:_bgScrollView];
    
    //分割线
//    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - 1, _btn_w * self.titleArray.count, 1)];
//    lineView.backgroundColor = [UIColor lightGrayColor];
//    [_bgScrollView addSubview:lineView];
...
//    [_bgScrollView addSubview:_selectLine];
    ...
    for (int i=0; i<self.titleArray.count; i++) {
        ...
        if (i==0) {
            _titleBtn=btn;
            btn.selected=YES;
            btn.backgroundColor = _bgSelectColor;
        }
    }
}
...
-(void)btnClick:(UIButton *)btn{
    ...
    if (btn.tag==_defaultIndex) {
        ...
        _titleBtn.backgroundColor = _bgNormalColor;
        _titleBtn.selected=!_titleBtn.isSelected;
        _titleBtn=btn;
        _titleBtn.selected=YES;
        _titleBtn.backgroundColor = _bgSelectColor;
...
- (void)setBgSelectColor:(UIColor *)bgSelectColor{
    _bgSelectColor = bgSelectColor;
...
-(void)updateView{
    for (UIButton *btn in _btns) {
        ...
        if (btn.tag-1 == _defaultIndex-1) {
            ...
            btn.selected=NO;
            btn.backgroundColor = _bgNormalColor;
        }
    }
}
