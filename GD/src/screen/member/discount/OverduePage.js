...
export default class OverduePage extends Component {
    ...
    componentDidMount() {
        RedPacketApi.fetchGetExired(UserInfo.user_id).then((res) =>{
            this.setState({
                data:res
            })
        })
    ...
    _renderItem=({item,index}) => {
       let validity_delta = item.description_map.validity_delta;
        ...
        return (
            <Row style={styles.itemStyle}>
                ...
                <Text mediumSize red text={validity_delta.indexOf('-') === 0 ? "已过期": item.description_map.validity_delta}/>
            </Row>
        ...
        return (
            <Row verticalCenter style={{justifyContent:'space-between' ,...paddingTB(15)}}>
                <Text smallSize> {'有 '}
                    ...
                    <Text smallSize black text={'个红包已到期'}/>
                </Text>
